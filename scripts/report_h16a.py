...
if seed:
 old=Path('research_log/H16A/full/artifacts/experiment')
 ...
 reference_first=json.loads((old/'local_seed0/rounds.jsonl').read_text().splitlines()[0])
 for i in range(10):assert first[0]['batch_hashes'][i]!=reference_first['batch_hashes'][i]
 if seed==2:
  previous_first=json.loads((base/'local_seed1/rounds.jsonl').read_text().splitlines()[0])
  for i in range(10):assert first[0]['batch_hashes'][i]!=previous_first['batch_hashes'][i]
...
assert seed in (0,1,2) and ownership_seed==120100
...
assert all(mm['owners_per_class']==1 and mm['k']==10 and mm['seed']==seed and mm['rounds']==10 and mm['batch_size']==32 and mm['local_epochs']==1 and mm['lr']==.01 and not mm['mixed_backbone'] for mm in meta.values())
if seed==0:assert meta['local']['initial_state_sha256']==read(oldbase/'local_seed0/metadata.json')['initial_state_sha256']
...
favg=[json.loads(x) for x in (base/f'fedavg_seed{seed}/rounds.jsonl').read_text().splitlines()]
for rr in favg:
...
communication={}
for mode in modes:
 rr=[json.loads(x) for x in (base/f'{mode}_seed{seed}/rounds.jsonl').read_text().splitlines()]
