...
root=Path(sys.argv[1]);seed=int(sys.argv[2]) if len(sys.argv)>2 else 0;base=root/'artifacts/experiment'
destination=root if seed==0 else root/f'seed{seed}';destination.mkdir(parents=True,exist_ok=True)
def read(p):return json.loads(Path(p).read_text(encoding='utf-8'))
modes=('local','fedproto','fedgh');runs={m:read(base/f'{m}_seed{seed}/final.json') for m in modes}
splits={m:read(base/f'{m}_seed{seed}/split.json') for m in modes};s=splits['local'];assert all(v==s for v in splits.values())
...
history=read(f'research_log/H02A/full/artifacts/experiment/fedgh_seed{seed}/split.json');assert s['class_sets']==history['class_sets']
...
first=[];runtime={};meta={}
for mode in modes:
 folder=base/f'{mode}_seed{seed}';rr=[json.loads(x) for x in (folder/'rounds.jsonl').read_text().splitlines()];assert len(rr)==10
...
lines=[f'# H11-A full-data seed{seed} frozen H07 scale validation','','| Arm | Seen % | Missing % | All % | Macro % | Classes |','|---|---:|---:|---:|---:|---:|']
...
(destination/'RESULTS.md').write_text('\n'.join(lines)+'\n',encoding='utf-8');(destination/'verification.json').write_text(json.dumps(verification,indent=2),encoding='utf-8')
